...
def DataPreprocessing(in_file_name, cust_dict, lag_cust_dict1, lag_cust_dict2, lag_cust_dict3, lag_cust_dict4,lag_cust_dict5,lag_cust_dict6,lag_cust_dict7,lag_cust_dict8,lag_cust_dict9,lag_cust_dict10,lag_cust_dict11):
    x_vars_list = []
    y_vars_list = []
    for row in csv.DictReader(in_file_name):
        ...
        if (row['fecha_dato'] in ['2015-06-28'] ):
            target_list = Process_products(row)
            lag_cust_dict5[cust_id] =  target_list[:]
            continue

                    
        if (row['fecha_dato'] in ['2015-07-28'] ):
            target_list = Process_products(row)
            lag_cust_dict6[cust_id] =  target_list[:]
            continue

                    
        if (row['fecha_dato'] in ['2015-08-28'] ):
            target_list = Process_products(row)
            lag_cust_dict7[cust_id] =  target_list[:]
            continue

        if (row['fecha_dato'] in ['2015-09-28'] ):
            target_list = Process_products(row)
            lag_cust_dict8[cust_id] =  target_list[:]
            continue
        if (row['fecha_dato'] in ['2015-10-28'] ):
            target_list = Process_products(row)
            lag_cust_dict9[cust_id] =  target_list[:]
            continue
        if (row['fecha_dato'] in ['2015-11-28'] ):
            target_list = Process_products(row)
            lag_cust_dict10[cust_id] =  target_list[:]
            continue
        ...
        if row['fecha_dato'] == '2016-06-28':
            ...
            lag_target_list5 = lag_cust_dict5.get(cust_id, [0]*22)
            lag_target_list6 = lag_cust_dict6.get(cust_id, [0]*22)
            lag_target_list7 = lag_cust_dict7.get(cust_id, [0]*22)
            lag_target_list8 = lag_cust_dict8.get(cust_id, [0]*22)
            lag_target_list9 = lag_cust_dict9.get(cust_id, [0]*22)
            lag_target_list10 = lag_cust_dict10.get(cust_id, [0]*22)
            lag_target_list11 = lag_cust_dict11.get(cust_id, [0]*22)



            x_vars_list.append(x_vars + prev_target_list + lag_target_list1+lag_target_list2 + lag_target_list3 + lag_target_list4+lag_target_list5+lag_target_list6+lag_target_list7+lag_target_list8+lag_target_list9+lag_target_list10+ lag_target_list11)
    ...
    return x_vars_list, y_vars_list, cust_dict, lag_cust_dict1, lag_cust_dict2, lag_cust_dict3, lag_cust_dict4,lag_cust_dict5,lag_cust_dict6,lag_cust_dict7,lag_cust_dict8,lag_cust_dict9,lag_cust_dict10,lag_cust_dict11
...
if __name__ == "__main__":
  ...
  x_vars_list, y_vars_list, cust_dict, lag_cust_dict1, lag_cust_dict2, lag_cust_dict3, lag_cust_dict4,lag_cust_dict5,lag_cust_dict6,lag_cust_dict7,lag_cust_dict8,lag_cust_dict9,lag_cust_dict10,lag_cust_dict11 = DataPreprocessing(train_file, {}, {}, {}, {}, {},{},{},{},{},{},{},{})
  ...
  x_vars_list, y_vars_list, cust_dict, _, _, _, _,_,_,_,_,_,_,_ = DataPreprocessing(test_file, cust_dict, lag_cust_dict1, lag_cust_dict2, lag_cust_dict3, lag_cust_dict4,lag_cust_dict5,lag_cust_dict6,lag_cust_dict7,lag_cust_dict8,lag_cust_dict9,lag_cust_dict10,lag_cust_dict11)
...
# In this solution, model is trained on 12 months of historical ownership features instead of 5 months.
# 
# **Final Result: 0.02453 on Private Score** 
